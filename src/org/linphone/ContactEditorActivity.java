...
import org.linphone.BootReceiver;
import org.linphone.Contact;
import org.linphone.ContactEditorFragment;
...
/**
 * Created by syz on 2016/10/21.
 */

public class ContactEditorActivity  extends Activity implements View.OnClickListener {

    private static final int ADD_PHOTO = 1337;

    private ImageView cancel;          //  取消操作
    private ImageView deleteContact;    //  删除操作
    private ImageView ok;              //   添加

    private ImageView addNumber;         // phone number
    private ImageView addSipAddress;     // sip地址
    private ImageView contactPicture;   //头像

    private EditText firstName;     // 姓氏
    private EditText lastName;     // 名字
...
*/
        isNewContact = true;
        deleteContact = (ImageView)findViewById(R.id.delete_contact);
        ...
        judgementIsNewContact(isNewContact);
        contactPicture = (ImageView)findViewById(R.id.contact_picture);
        if (contact != null && contact.getPhotoUri() != null) {
            InputStream input = Compatibility.getContactPictureInputStream(this.getContentResolver(), contact.getID());
        ...
        addNumber = (ImageView) findViewById(R.id.add_number_field);
        ...
        addSipAddress.setOnClickListener(this);
    ...
    public void   judgementIsNewContact(boolean isNewContact ) {
                ...
                public void onClick(View v) {
                    ...
                    delete.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            ...
                            ContactsManager.getInstance().removeContactFromLists(getContentResolver(), contact);
    ...
    public void onClick(View view) {
        ...
        switch (i) {
            ...
            case R.id.ok:
                okInfo(isNewContact);
                break;
            case R.id.contact_picture:   //头像
                pickImage();
                break;
            case R.id.add_address_field:  //sip address_
                addEmptyRowToAllowNewNumberOrAddress(sipAddresses,true);
                break;
            case R.id.add_number_field:  // phone  number
    ...
    *   ok(Button)
    * */
    public void  okInfo(boolean isNewContact) {
    ...
    private LinearLayout initNumbersFields(final Contact contact) {
        LinearLayout controls = (LinearLayout)findViewById(R.id.controls_numbers);
    ...
    private View displayNumberOrAddress(final LinearLayout controls, String numberOrAddress, boolean forceAddNumber) {
        ...
        final View view = LayoutInflater.from(this).inflate(R.layout.contact_edit_row, null);
            ...
            if(numberOrAddress.newNumberOrAddress != null && numberOrAddress.isSipAddress) {
                if(isNewContact){
                    ...
                    if (c != null && contactsManager.isContactHasAddress(c, numberOrAddress.newNumberOrAddress)) {
                        ...
                        Log.i("newNumberOrAddress1:-------------------------------->"+numberOrAddress.newNumberOrAddress);
                    }
                } else {
   ...
   class NewOrUpdatedNumberOrAddress{
       ...
       private void addNewNumber() {
           ...
           if (isNewContact) {
               if (isSipAddress) {
                   ...
                   Compatibility.addSipAddressToContact(ContactEditorActivity.this, ops, newNumberOrAddress);
               } else {
                   ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
               ...
               String rawContactId = contactsManager.findRawContactID(ContactEditorActivity.this.getContentResolver(),String.valueOf(contactID));
               if (isSipAddress) {
                   if (newNumberOrAddress.startsWith("sip:"))
                       newNumberOrAddress = newNumberOrAddress.substring(4);
                   if(!newNumberOrAddress.contains("@")) {
                   ...
                   Compatibility.addSipAddressToContact(ContactEditorActivity.this, ops, newNumberOrAddress, rawContactId);
                   if (getResources().getBoolean(R.bool.use_linphone_tag)) {
                       Compatibility.addLinphoneContactTag(ContactEditorActivity.this, ops, newNumberOrAddress, contactsManager.findRawLinphoneContactID(String.valueOf(contactID)));
                   }
               } else {
                   ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
                           .withValue(ContactsContract.Data.RAW_CONTACT_ID, rawContactId)
       ...
       private void updateNumber() {
           ...
           if (isSipAddress) {
               ...
               if (getResources().getBoolean(R.bool.use_linphone_tag)) {
                   Compatibility.updateLinphoneContactTag(ContactEditorActivity.this, ops, newNumberOrAddress, oldNumberOrAddress, contactsManager.findRawLinphoneContactID(String.valueOf(contactID)));
               }
           } else {
               String select = ContactsContract.Data.CONTACT_ID + "=? AND "
                       + ContactsContract.Data.MIMETYPE + "='" + ContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE +  "' AND "
