...
public class LinphonePreferences {
	...
	public static class AccountBuilder {
		...
		public void saveNewAccount() throws LinphoneCoreException {
			...
			//lifeng:去掉多余的配置项,否则在同一台手机上使用不同的帐号登录,会出问题
			for (LinphoneProxyConfig proxyConfig : lc.getProxyConfigList()) {
				lc.removeProxyConfig(proxyConfig);
			}
			for (LinphoneAuthInfo authInfo1 : lc.getAuthInfosList()) {
				lc.removeAuthInfo(authInfo1);
			}
