...
//		setContentView(R.layout.launch_screen);
        setContentView(R.layout.logindex);
	...
	protected void onNewIntent(Intent intent) {
		...
		code = intent.getStringExtra("passwd");
//		Log.i("user1:--------------------------->"+username);
//		Log.i("code1:--------------------------->"+code);
	...
	protected void onResume() {
		...
		try {
			Bundle bundle = intent.getBundleExtra("usermessage");
			username = bundle.getString("username");
			code = bundle.getString("password");
		} catch (Exception e) {
			e.printStackTrace();
		}
		super.onResume();
	...
	protected void onServiceReady() {
		...
		mHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				Intent intent = new Intent().setClass(LinphoneLauncherActivity.this, classToStart).setData(getIntent().getData());
				intent.putExtra("username", username).putExtra("password", code);
