...
//            displayRegistrationInProgressDialog();
    ...
    private static final String sip_domain = "116.228.237.226";

    private String getCallerIdFromUserName(String username) {
        if (username == "test1") {
            return "1001";
            ...
            return "1004";
        } else if (username == "test5") {
            return "1005";
        } else if (username == "test6") {
            return "1006";
        } else if (username == "test7") {
            return "1007";
        }
        else {
            return null;//error
...
*/
        ...
        code = getIntent().getStringExtra("password");
        Log.i("user3:------------------------------>" + username);
        Log.i("code3:------------------------------>" + code);
        Log.i("deviceid:---------------------------->"+ deviceId);
        saveCreatedAccount(username, code, username, sip_domain, LinphoneAddress.TransportType.LinphoneTransportUdp);
        //TODO rework
...
//				selectMenu(FragmentsAvailable.DIALER);
        ...
        mListener = new LinphoneCoreListenerBase() {
            ...
            public void registrationState(LinphoneCore lc, LinphoneProxyConfig proxy, LinphoneCore.RegistrationState state, String smessage) {
                ...
                //lifeng: 注册失败的情况下给出提示
                Log.i("registration state@@@@@@@@@@@@@@@@@@"+state);
                if (state.equals(RegistrationState.RegistrationFailed)) {
                    displayCustomToast("注册失败:账户错误或者被占用", Toast.LENGTH_LONG);
                    finish();
                    boolean isBackgroundModeActive = LinphonePreferences.instance().isBackgroundModeEnabled();
                    if (!isBackgroundModeActive) {
                        stopService(new Intent(Intent.ACTION_MAIN).setClass(LinphoneActivity.this, LinphoneService.class));
                    }
                } else if (state.equals(RegistrationState.RegistrationOk)) {
                    onEnterConference(true);
                }
            }

            @Override
            public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
                //lifeng:将calloutgoingactivity.java里的CALLSTATE()内容合并进来
...
//					startActivity(new Intent(LinphoneActivity.instance(), CallOutgoingActivity.class));
                } else if (state == State.StreamsRunning/* || state == State.Connected*/) {
                    if (!CallActivity.isInstanciated()) {
                        Log.i("startInCallActivity5:----------------------------------");
...
//                        displayCustomToast(getString(R.string.error_unknown) + " - " + message, Toast.LENGTH_SHORT);
                        ...
                        for (LinphoneCall call1 : lc.getCalls()) {
                            if (call1.getState() == State.Paused)
                                lc.resumeCall(call1);
                        }
                        if (LinphoneManager.getLcIfManagerNotDestroyedOrNull().getCallsNb() == 0) {
/*
                            conference.setText("断线,原因可能是:登录时账户错,网络不好,其他人用相同帐号" +
                                    "登录将您踢下" +
                                    "想继续使用的话,请退出程序输入正确帐号并登录");
                            conference.setTextSize(20);
*/
                            if (LinphoneManager.getLcIfManagerNotDestroyedOrNull().isNetworkReachable()) {
//                                LinphoneManager.getInstance().newOutgoingCall("3100", null);
                            }
                            else {
                                conference.setText("网络断线,请恢复网络连接");
                                conference.setTextSize(20);
                                LinStatText.setText("网络断线,请恢复网络连接");
                            }
                        }
		...
		conference = (TextView) findViewById(R.id.btn_conference);
        LinStatText = (TextView) findViewById(R.id.mainsynctext);
/*
...
*/
        ...
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                onEnterConference(false);
            }
        }, 500);
        LinphonePreferences.instance().setBackgroundModeEnabled(true);     //lifeng add
    }

    public void onEnterConference(boolean do_it) {
        if (do_it) {
