...
//				}
		...
		pttFilter.addAction("android.yview.action.YV_BUTTON_PTT");
		myPTTReceiver = new MyPTTReceiver();
		registerReceiver(myPTTReceiver, pttFilter);
	...
	private MyPTTReceiver myPTTReceiver;
	...
	private final static int MOTION_EVENT_FROM_MEDIABUTTON = 77;
	public void toggleSpeakState() {
		isSpeaking = !isSpeaking;
	}


	private class MyPTTReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (isSpeaking == false) {
				...
				try {
					speakButton = CallActivity.instance().getSpeakRequest();
					me = MotionEvent.obtain(SystemClock.uptimeMillis(),
                            SystemClock.uptimeMillis(),
                            MotionEvent.ACTION_DOWN,
                            speakButton.getX(),
                            speakButton.getY(),
                            MOTION_EVENT_FROM_MEDIABUTTON);
				} catch (Exception e) {
				...
				try {
					...
					LinphoneManager.getLc().sendDtmf('D');    //'d'表示放开对讲按钮
					e.printStackTrace();
				}
				Log.i("released@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
				toggleSpeakState();
			...
			}, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK) == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
				audioManager.adjustStreamVolume(AudioManager.STREAM_MUSIC,
						AudioManager.ADJUST_RAISE,
						0);
				mp.start();
				try {
					speakButton.dispatchTouchEvent(me);
				} catch (Exception e) {
					Log.i("isSpeak()@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+isSpeaking());
					e.printStackTrace();
				}
				if (isSpeaking()) {
					...
					LinphoneManager.getLcIfManagerNotDestroyedOrNull().muteMic(true);
				}
			}
