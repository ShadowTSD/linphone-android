...
static void ui_config_read(LinphoneCore *lc)
{
	LinphoneFriend *lf;
	int i;
	//lifeng: 下面在ANDROID LINPHONE不需要,注释掉
	/*for (i=0;(lf=linphone_friend_new_from_config_file(lc,i))!=NULL;i++){
		linphone_core_add_friend(lc,lf);
		linphone_friend_unref(lf);
		ms_error("linphonecore.c:------------------------------------");
	}*/
				   ...
				   LinphonePresenceModel *presence)
{
	...
	char *current_uri=linphone_address_as_string_uri_only(linphone_presence_model_get_presentity(presence));
	ms_message("presenceModel:----------------------------->%s\n", current_uri);
...
#if 0
    LinphoneCall *call = linphone_core_get_current_call(lc);
    const LinphoneAddress *r_addr = linphone_call_get_remote_address(call);
    char *current_uri=linphone_address_as_string_uri_only(r_addr);
    ms_message("current_uri:-------------------------->%s\n", current_uri);
    if (strstr(current_uri, "sip:3100") != NULL && os == LinphoneStatusOnThePhone) {
        os = LinphoneStatusOnline;
